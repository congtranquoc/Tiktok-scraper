...
class scrape:
    def __init__(self, soup):
        ...
        self.postcontent = soup.find(tiktok.postcontent.element, {tiktok.postcontent.attribute:tiktok.postcontent.attval})

        try:
            self.likes = soup.find(tiktok.likes.element, {tiktok.likes.attribute:tiktok.likes.attval}).text

            self.saves = soup.find(tiktok.saves.element, {tiktok.saves.attribute: tiktok.saves.attval}).text

            self.shares = soup.find(tiktok.shares.element, {tiktok.shares.attribute: tiktok.shares.attval}).text

            self.browseUser = soup.find(tiktok.browseUser.element, {tiktok.browseUser.attribute: tiktok.browseUser.attval})['href'].replace('/','https://www.tiktok.com/')

            self.ownerName = soup.find(tiktok.ownerName.element, {tiktok.ownerName.attribute: tiktok.ownerName.attval}).text
        except:
            print("It is not profile page")
